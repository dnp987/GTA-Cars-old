package Test1;
// Logs into BMO IL, verifies the home page, the main menu, then gets Canadian and US equity quotes
...
public class QuotesTest_CA_US {
    ...
    public String passWd = "";
     ...
     public void verify_quotes() {
    	...
    	data_sheet_out.setCell(0, heading_data, dataFont, boldOn, largeFont); // add the headings in row #0
    	
   	   String testSymbol = "";
   	   String testExchange = "";
    	     	 
    	 for (int i = startrow+1; i <= endrow; i++) {  // get a quote for each symbol in the spreadsheet and save the quote data in another spreadsheet
    		 ArrayList<String> quote_data = new ArrayList<String>(); // reset the quote_data for each row
    		 testSymbol = test_symbols.getCell(i, 0);
    		 testExchange = test_symbols.getCell(i,1);
    	     ...
    	     /*Select drpCountry = new Select(driver.findElement(By.cssSelector("div.paddingLeft25:nth-child(3) > select:nth-child(2)"))); //select the country drop down list
    	     drpCountry.selectByVisibleText(testExchange); // set the exchange according to the symbol read in */
    	     new Select(driver.findElement(By.cssSelector("div.paddingLeft25:nth-child(3) > select:nth-child(2)"))).selectByVisibleText(testExchange);	     
	    	 ...
	    	 data_sheet_out.setCell(i, quote_data, dataFont, boldOff, smallFont);
    	 }
    	 data_sheet_out.createDataOut("c:\\temp\\quotes_test-java.xlsx"); 
     }
